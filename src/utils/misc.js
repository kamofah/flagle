import { fetchCountriesData, fetchCountryNames, } from '../api/country';
...
export const selectCountry = async () => {
  ...
  const continent = formatContinentText(selectedCountry.continents);
  ...
  return [countryName, countryFlag, continent, countryLanguage];
};

export const getListOfCountries = async () => {
  const countryNamesResponse = await fetchCountryNames();
  const listOfCountries = countryNamesResponse.map((countryName) => {
    return countryName.name.common;
  });
  return listOfCountries;
};

export const validateUserAttempt = (userAttempt, correctSolution, currentAttemptNumber, colorMap, colorVariable) => {
  if(userAttempt === correctSolution){
    colorMap[currentAttemptNumber][colorVariable] = '#50C878';
  } else {
    colorMap[currentAttemptNumber][colorVariable] = '#DC143C';
  }
};
...
export const calcDistanceBetweenCordinates = (lat1, lon1, lat2, lon2) => {
  ...
  var a = Math.sin(dLatInRad/2) *
    Math.sin(dLatInRad/2) +
